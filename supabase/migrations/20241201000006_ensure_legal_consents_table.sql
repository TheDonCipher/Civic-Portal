-- Ensure Legal Consents Table Migration for Civic Portal
-- This migration ensures the legal_consents table exists with all required fields and proper RLS
...
DROP POLICY IF EXISTS "Admins can view all consent records" ON legal_consents;
...
-- Admins can view all consent records for compliance (will be enabled after profiles table exists)
...
DO $$
BEGIN
  RAISE NOTICE 'Legal consents table ensured with all required fields and RLS policies';
END $$;
