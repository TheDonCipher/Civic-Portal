    ...
    try {
      ...
      try {
        const { count, error: openError } = (await supabase
          .from('issues')
          .select('*', { count: 'exact', head: true })
          .eq('status', 'open')) as PostgrestCountQueryResult<any>;
      ...
      } catch (openFetchError) {
        console.error('Failed to fetch open issues:', openFetchError);
      }

      // Get in-progress issues count
      let inProgressCount = 0;
      try {
        const { count, error: inProgressError } = (await supabase
          .from('issues')
          .select('*', { count: 'exact', head: true })
          .eq('status', 'in-progress')) as PostgrestCountQueryResult<any>;

        if (inProgressError) {
          console.error('Error fetching in-progress issues:', inProgressError);
        } else {
          inProgressCount = count || 0;
        }
      } catch (inProgressFetchError) {
        console.error(
          'Failed to fetch in-progress issues:',
          inProgressFetchError
        );
      }
      ...
      const openIssues = openCount || 0;
      const inProgressIssues = inProgressCount || 0;
